...
BEETS_LIBRARY_PATH = "/config/musiclibrary.db"
BEETS_LOG_PATH = "/config/logs/beets-import.log"
...
CONFIG_OUTPUT_FILE = RUNTIME_DIR / "config.generated.yaml"
SUBSCRIPTIONS_OUTPUT_FILE = RUNTIME_DIR / "subscriptions.generated.yaml"
BEETS_OUTPUT_FILE = RUNTIME_DIR / "beets.music-playlist.yaml"
...
def configure_runtime_context(project_root: Path | None, mode: str, downloads_root: str | None) -> None:
    ...
    global DOWNLOADS_ROOT, BEETS_LIBRARY_PATH, BEETS_LOG_PATH
    global RUNTIME_DIR, PROFILES_FILE, SUBSCRIPTIONS_FILE, CONFIG_OUTPUT_FILE
    global SUBSCRIPTIONS_OUTPUT_FILE, BEETS_OUTPUT_FILE
    ...
    BEETS_LIBRARY_PATH = settings.beets_library_path
    BEETS_LOG_PATH = settings.beets_log_path
...
def log(level: str, message: str) -> None:
    ConsoleTheme.print(level, f"[{now_str()}] [{level}] {message}")
...
def fail(message: str) -> None:
    ConsoleTheme.print("ERROR", f"[{now_str()}] [ERROR] {message}", file=sys.stderr)
    sys.exit(1)
...
def build_recent_ytdl_options(max_items: int, *, break_on_existing: bool = True) -> dict[str, Any]:
    if max_items <= 0:
        return {}
    return {
...
def preferred_mp3_source_format() -> str:
    return "ba[acodec^=mp3]/ba/b"
...
def build_profile_preset(profile_type: str, merged_values: dict[str, Any]) -> dict[str, Any]:
    ...
    if profile_key == "canales-youtube":
        ...
        tv_show_directory = merged_values.get(
            "tv_show_directory",
            f"{DOWNLOADS_ROOT}/Canales-youtube",
        )
        ...
        return {
            ...
            "ytdl_options": build_recent_ytdl_options(max_items),
        }

    if profile_key == "podcast":
        output_directory = merged_values.get(
            "output_directory",
            f"{DOWNLOADS_ROOT}/Podcast/{{subscription_root_sanitized}}/{{source_target_sanitized}}",
        )
        ...
        return {
            ...
            "download": "{url}",
            "format": preferred_mp3_source_format(),
            "audio_extract": {
                "enable": True,
                "codec": "mp3",
                "quality": 0,
    ...
    if profile_key == "tv-serie":
        ...
        tv_show_directory = merged_values.get(
            "tv_show_directory",
            f"{DOWNLOADS_ROOT}/TV-Serie",
        )

        ytdl_options = build_recent_ytdl_options(max_items)
        if output_format:
            ytdl_options["merge_output_format"] = output_format
    ...
    if profile_key == "music-playlist":
        output_directory = merged_values.get(
            "output_directory",
            f"{DOWNLOADS_ROOT}/Music-Playlist/{{subscription_root_sanitized}}",
        )
    ...
    if profile_key == "ambience-video":
        output_directory = merged_values.get(
            "output_directory",
            f"{DOWNLOADS_ROOT}/Ambience-Video/{{subscription_root_sanitized}}",
        )
    ...
    if profile_key == "ambience-audio":
        output_directory = merged_values.get(
            "output_directory",
            f"{DOWNLOADS_ROOT}/Ambience-Audio/{{subscription_root_sanitized}}",
        )
...
def build_beets_config() -> dict[str, Any]:
    return {
        "directory": f"{DOWNLOADS_ROOT}/Music-Playlist",
...
def main() -> None:
    ...
    parser.add_argument("--only-profile", dest="only_profile", default="", help="Genera solo un profile_type, por ejemplo: music-playlist")
    ...
    try:
        ...
        config_generated: dict[str, Any] = {
            "configuration": {
                ...
                "persist_logs": {"keep_successful_logs": True, "logs_directory": "/config/logs"},
                "lock_directory": "/tmp",
            },
            "presets": {},
        }
        ...
        for subscription in subscriptions:
            ...
            if profile_name not in profiles:
                raise ValueError(f"La suscripción usa un perfil no definido: {profile_name}")
            ...
            for source in subscription["sources"]:
                ...
                preset_name, subscription_name, entry = build_subscription_entry(profile_type=profile_type, custom_name=custom_name, merged_values=merged_values, source_item=source)
                ...
                config_generated["presets"][preset_name] = build_profile_preset(profile_type=profile_type, merged_values=merged_values)
                subscriptions_generated[preset_name] = {f"~{subscription_name}": entry}
        ...
        info(f"Modo         : {MODE}")
        info(f"Config user  : {CONFIG_USER_DIR}")
        info(f"Runtime dir  : {RUNTIME_DIR}")
        info(f"Downloads    : {DOWNLOADS_ROOT}")
        info(f"Generado: {CONFIG_OUTPUT_FILE}")
        info(f"Generado: {SUBSCRIPTIONS_OUTPUT_FILE}")
        if includes_music_playlist:
            info(f"Generado: {BEETS_OUTPUT_FILE}")
    except Exception as exc:
        fail(str(exc))
